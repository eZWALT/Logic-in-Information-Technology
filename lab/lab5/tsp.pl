...
%%% SI NO QUEDEN CIUTATS PER VISITAR, ESTEM A UNA FULLA, CAL CONSULTAR SI AQUESTA SOLUCIO ES MILLOR QUE LA ANTERIOR MILLOR if(newmin < min) min = newmin
...
%%TSP LOWER BOUND GEEKSFORGEEKS
lowerBoundOfRemainingCities(Cities, LBound):- sumatorio(..., LBound).
...
%myselect 
myselect(CurrentCity, DesitinationCity, Cities, RemainingCities):-
	findminimum

myselect(CurrentCity, DesitinationCity, Cities, RemainingCities):- 
	select(City1, Cities, NewRemainingCities),  

findminimum(CCity,Cities, MinDist):- findall(Km, (member(City,Cities), distance(CCity, City, Km) ), dists ), min_list(dists, MinDist).
