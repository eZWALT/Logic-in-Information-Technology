...
defineIsActive(MaxHours):-
    task(T), between(1,MaxHours,H),
    duration(T,D),between(0,D,I), I2 is H + I,
    writeClause([-start(T,H),isActive(T,I2)]),
    fail.
defineIsActive(_).
...
eachTaskStartsOnce(MaxHours) :-
    ...
    duration(T,D),
    Hl is MaxHours - D + 1,
    findall(start(T,H),between(1,Hl,H), Lits), 
    exactly(1,Lits),
    fail.
eachTaskStartsOnce(_).

atMostkResourcesInHour(MaxHours) :-
    resourceUnits(R,U), between(1,MaxHours,H),
