...
connect(C, C2) :- city(C), city(C2), (road(C,C2) ; road(C2, C)).
...
allpair :- connect(C,C2), writeClause([install(C), install(C2)]), fail. 
allpair.

mostK :- findall(install(C), city(C), L),maxStations(K), atMost(K,L).

%%%%%%  3. DisplaySol: show the solution. Here M contains the literals that are true in the model:

%displaySol(M):- nl, write(M), nl, nl, fail.
displaySol(M):- write('Ciudades: '), nl, city(C), member(install(C), M), write(C), nl, fail.
