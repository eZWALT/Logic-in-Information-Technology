...
isAdj(C, C2) :- adjacency(C, L), member(C2,L).
posnotdet(P) :- numCities(N), M is N-1, between(1,M,P).
% adyacente de la forma C -> C2 (C llego a C2)
%%%%%%% =======================================================================================
%
...
startAndEndOne :- numCities(K), writeClause([visited(1,0)]), writeClause([visited(1,K)]).

todasciudades :- city(I), I =\= 1, findall(visited(I,P), posnotdet(P), L),writeClause(L), fail.
todasciudades.

withNoRep :- position(P), position(Q), P =\= Q, city(I), I =\= 1, writeClause([-visited(I,P), -visited(I,Q)]), fail.
withNoRep.

adyacente :- position(P), numCities(K), P < K, Q is P + 1, position(Q), city(I), city(J), I =\= J, \+isAdj(I,J),writeClause([-visited(I,P), -visited(J,Q)]),fail.
adyacente.

visitonetime :- city(I), city(J), position(P), I =\= J, writeClause([-visited(I,P), -visited(J,P)]), fail.
visitonetime.
