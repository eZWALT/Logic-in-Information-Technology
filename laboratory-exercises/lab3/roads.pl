...
connect(C1,C2) :- city(C1), city(C2), (road(C1,C2) ; road(C2,C1)).
...
writeClauses:- allpairs, notTooManyStations, true, !.
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

notTooManyStations:-  findall(install(C), city(C),Lit), maxStations(K) ,atMost(K,Lit), fail.
notTooManyStations.


allpairs:- connect(C1,C2), writeClause([install(C1), install(C2)]), fail.
allpairs.

%%%%%%  3. DisplaySol: show the solution. Here M contains the literals that are true in the model:

%displaySol(M):- nl, write(M), nl, nl, fail.
displaySol(M):- write('Ciudades: '),
                  nl ,
                  city(C),
                  member(install(C), M),
                  write(C),
