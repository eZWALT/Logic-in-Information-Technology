...
%%% 1. minimoDeZorrasDiarias, 2.RespetarHorariosQueNoPueden,3. Intercalaras (trabajan3-1-3)
...
writeClauses:-
	cadaHoraMinimoKzorras,
	cadaZorraTe1Tipus,
	cadaZorraComenca1Cop,
	esRespectenHoresBloquejades,
	esRespectenHoresDeTreball,
	esRespectenHoresDeDescans,
    true.)

cadaHoraMinimoKzorras:- hour(H), needed(H,K) ,findall(worksNH-N-H, nurse(N),Lits), atLeast(K,Lits),fail.
cadaHoraMinimoKzorras.

cadaZorraTeTipus:- nurse(N), findall(nurseType-N-Type, type(N), Lits), exactly(1,Lits),fail.
cadaZorraTeTipus.

cadaZorraComenca1Cop:- nurse(N), findall(startsNH-N-H, hour(H), Lits), exactly(1,Lits), fail.
cadaZorraComenca1Cop.

esRespectenHoresBloquejades:- nurseIDandBlocking(N,B1,B2), hourInRange(B1,B2,H), negate(worksNH-N-H,Lits),writeClause([Lits])  fail.
esRespectenHoresBloquejades.

esRespectenHoresDeTreball :- nurse(N), hour(hstart), type(T), hour(hend), workingHourForTypeAndStartH(T,hstart,hend), writeClause([-startsNH-N-hstart,-nurseType-N-T,worksNH-N-hend]),fail.
esRespectenHoresDeTreball.

esRespectenHoresDeDescans :-  nurse(N), hour(Hstart), type(T), hour(HnotWork), \+workingHourForTypeAndStartH(T, Hstart, HnotWork),
