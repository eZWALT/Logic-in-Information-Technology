...
exactlyKStudents :- numMembersTeam(K), findall(chosen(S), student(S), Lits), exactly(K, Lits), fail.
exactlyKStudents.

excludeNotMutuals :- student(I), student(J), J =\= I, \+mutualFriends(I, J), atMost(1, [chosen(I), chosen(J)]), fail.
excludeNotMutuals.

mutualFriends(I, J) :- student(I), student(J), friends(I,J), friends(J,I).

excludeSameTopic :- expertise(_,E), findall(chosen(S), expertise(S, E), Lits), atMost(1, Lits), fail.
excludeSameTopic.

mutualTopic(I,J) :-student(I), student(J), expertise(I, E), expertise(J, E).
