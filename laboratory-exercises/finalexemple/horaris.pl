...
%%cada curso necesita un profe
eachCourseExactlyOneProf:- courseProfessors(C,LP), findall( cp(C,P), member(P,LP), Lits ), exactly(1,Lits), fail.
eachCourseExactlyOneProf.

%%cada curso usa una sala
eachCourseExactlyOneRoom:- courseRooms(C,RP), findall( cr(C,R), member(R,RP), Lits), exactly(1,Lits), fail.
eachCourseExactlyOneRoom.

%%para cada dia maximo una hora
eachCourseMaxOncePerDay:-  course(C), day(D),  findall(cdh(C,D,H), hour(H), Lits), atMost(1,Lits), fail.
eachCourseMaxOncePerDay.

%%si ese dia hay clase del curso pro1, alguna hora habra
relateVarsCDHwithCD:- course(C), day(D), findall(cdh(C,D,H), hour(H),Lits), expressOr(cd(C,D),Lits), fail.
relateVarsCDHwithCD.

%%cada curso debe tener exactamente Hs classes 
eachCourseRightNumberOfDays:- course(C), findall(cd(C,D), day(D),Lits), courseHours(C,Hs), exactly(Hs,Lits), fail.
eachCourseRightNumberOfDays.

%%esta clausula impide que 2 cursos del mismo año (IC i PRO1) se solapen, ya que seria impsosible ir a ambas clases
noOverlapCoursesSameYear:- courseYear(C1,Y), courseYear(C2,Y), C1 \= C2,day(D), hour(H), writeClause([-cdh(C1,D,H),-cdh(C2,D,H)]), fail. %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
noOverlapCoursesSameYear.

%%esta clausula impide que 2 cursos que se dan a la misma hora sean dados por el mismo profesor
noOverlapProfs:- course(C1), course(C2), professor(P),day(D),  hour(H), C1 \= C2,  writeClause([ -cdh(C1,D,H), -cdh(C2,D,H), -cp(C1,P), -cp(C2,P) ]), fail. 
noOverlapProfs.

%%esta clausula impide que 2 cursos que usan la misma aula R, puedan 
noOverlapRooms:- course(C1),course(C2), C1 \= C2, room(R), hour(H), day(D), writeClause([-cr(C1,R),-cr(C2,R),-cdh(C1,D,H),-cdh(C2,D,H)]), fail.
noOverlapRooms.
